...
class TestTower extends Node:
	...
	func has_method(method_name: StringName) -> bool:
		return method_name in ["reduce_cooldown", "add_ammo"]
	
	func install_module(module: Module) -> void:
		# 模拟模块安装：添加 effects
		bullet_effects.append_array(module.bullet_effects)
		tower_effects.append_array(module.tower_effects)
...
func test_cd_reduce_on_enemy_full_chain() -> void:
	...
	assert_float(tower.reduce_cooldown_calls[0]).is_equal(0.5)  # reduction 值
...
func test_replenish_effect_full_chain() -> void:
	...
	# 创建目标塔（会被子弹击中的塔）
	var target_tower := auto_free(TestTower.new())
	target_tower.ammo = 3
	...
	assert_int(target_tower.add_ammo_calls[0]).is_equal(1)  # ammo_amount
...
func test_module_install_adds_stat_modifiers() -> void:
	...
	if not module:
		skip("accelerator module not found")
		return
