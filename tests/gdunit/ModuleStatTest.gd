...
func test_accelerator_increases_bullet_speed() -> void:
	...
	var before: float = tower.get_stat(TowerStatModifierRes.Stat.BULLET_SPEED).get_value()
	assert_float(before).is_equal(200.0)

	tower.install_module(module)
	var after: float = tower.get_stat(TowerStatModifierRes.Stat.BULLET_SPEED).get_value()
...
func test_multiplier_scales_bullet_attack() -> void:
	...
	var before: float = tower.get_stat(TowerStatModifierRes.Stat.BULLET_ATTACK).get_value()
	assert_float(before).is_equal(1.0)

	tower.install_module(module)
	var after: float = tower.get_stat(TowerStatModifierRes.Stat.BULLET_ATTACK).get_value()
...
func test_rate_boost_reduces_cd() -> void:
	...
	var before: float = tower.get_stat(TowerStatModifierRes.Stat.CD).get_value()
	assert_float(before).is_equal(1.0)

	tower.install_module(module)
	var after: float = tower.get_stat(TowerStatModifierRes.Stat.CD).get_value()
...
func test_heavy_ammo_scales_bullet_attack() -> void:
	...
	tower.install_module(module)
	var attack: float = tower.get_stat(TowerStatModifierRes.Stat.BULLET_ATTACK).get_value()
...
func test_heavy_ammo_ammo_extra_after_fix() -> void:
	...
	tower.install_module(module)
	var ammo_extra: float = tower.get_stat(TowerStatModifierRes.Stat.AMMO_EXTRA).get_value()
