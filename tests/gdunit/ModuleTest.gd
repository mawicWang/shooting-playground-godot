...
# 运行方式: 在 Godot 编辑器中点击 GdUnit 面板运行
...
func test_module_uninstall_removes_effects_from_tower() -> void:
	...
	# 验证速度恢复
	var final_speed: float = tower.get_bullet_speed()
...
func test_multiple_modules_stacking() -> void:
	...
	var initial_attack: float = tower.get_bullet_attack()
	...
	var final_attack: float = tower.get_bullet_attack()
...
func test_module_slot_limit() -> void:
	...
	# 安装 4 个模块
	for i in range(4):
		var success: bool = tower.install_module(module.duplicate())
		assert_bool(success).is_true()

	assert_that(tower.get_module_count()).is_equal(4)

	# 第 5 个应该失败
	var fail_result: bool = tower.install_module(module.duplicate())
